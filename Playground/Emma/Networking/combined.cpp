...
int main() {
    ...
    int receieveSocket;
    receieveSocket = socket(res->ai_family, res->ai_socktype, res->ai_protocol);
    if (receieveSocket == -1) {
        perror("socket");
        exit(1);
    }
    ...
    // Turn on reuse port
    int one = 1;
    if (setsockopt(receieveSocket, SOL_SOCKET, SO_REUSEPORT, &one, sizeof(int)) < 0) {
        perror("setsockopt");
    }

    if (bind(receieveSocket, res->ai_addr, res->ai_addrlen) < 0) {
